...
import { setPlayVideoIndex, setPageNum } from '../../actions/globalData'
import { throttle, paging } from '../../util/util'
    ...
    const videos = commandList.map((el, i) => {
      return <View className="recommendedLogo" key={i}>
              <RecommendedLogo
                ...
                poster ={el['Suggestworksinfo']['coverImg']}
                videoSrc= {el['Suggestworksinfo']['videoUrl']}
                likeNum= {el['Suggestworksinfo']['priceCount']}
                title= {el['Suggestworksinfo']['worksName']}
                curNumber={i+1}
                workerId={el['Suggestworksinfo']['worksId']}
              >
              </RecommendedLogo>
            </View>
    })
