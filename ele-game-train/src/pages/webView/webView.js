...
import { View, Image, Text, Video, WebView } from '@tarojs/components'
...
class webView extends Component {
    ...
    switchUrl() {
        ...
        switch (pageName) {
            case 'index':            // 预览页
                src = WEB_API.webviewUrl
                    + 'scratch/index.html?workId=' + this.$router.params.id
                    + '&token=' + this.props.authData.token
                    + '&fullscreen=true&sourceh5=miniProgram'
                    + authorId;
                break;
            case 'allworks':         // 个人主页
                src = WEB_API.webviewUrl
                    + 'activity/allworks/allworks.html?studentId=' + this.$router.params.id
                    + '&token=' + this.props.authData.token
                    + '&source=miniProgram'
        }
        this.setState({
    ...
    handleMsg(e) {
        ...
        if (data.clockIn) { clockIn = data.clockIn };
        shareMsg = {
            authorid: authorid ? '&authorid=' + authorid : '',
            imgUrl,
            title,
    ...
    render() {
        return (
            <WebView src={this.state.src} onMessage={this.handleMsg}></WebView>
        )
    }
