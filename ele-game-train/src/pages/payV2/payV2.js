...
class PayV2 extends Component {
    ...
    toPay() {
        ...
        if(isMulti) redirectTo('payChoice', `gdt_vid=${gdt_vid}&adId=${adId}`);
        else {
    ...
    handlePayButtonClick() {
        ...
        if(isBuy) redirectTo('paySuccess');
        else if(isIOS) {
            this.setState({
                isModalGuideShow: true,
                modalTitle: '立即报名'
    ...
    render() {
        ...
        const introImgList = [...Array(12).keys()].map(index => {
            return (
                <View
                    key={index}
                    onClick={this.handlChangeImgIndex2.bind(this, index)}
                    className={index === 0 ? 'intro-course-info' : index === 3 || index === 7 || index === 8 ? 'intro-video' : ''}
                >
                    ...
                    {index === 2 && <Image
                        ...
                        src={WEB_API.buyImageUrl+'13.png'}
                        style={{display: imgIndex2 ? 'block' : 'none'}}
                        mode="widthFix"
                    />}
                    ...
                    {index === 8 && <View className="gifimage-wrapper">
                        <Image
                            className="intro-image-gif"
                            src={WEB_API.buyVideoUrl+'program.gif'}
                            mode="widthFix"
                        />
                        <Image
                            className="intro-image-gif intro-image-gif1"
                            src={WEB_API.buyVideoUrl+'code.gif'}
                            mode="widthFix"
                        />
                    </View>}
                    <Image
                        className="intro-image"
                        src={WEB_API.buyImageUrl+(index+1 > 9 ? '' : '0')+(index+1)+'.png'}
                        mode="widthFix"
                    />
                </View>
            );
            ...
            <View className="wrapper">
                ...
                <View className="bottom-bar">
                    {!isBuy && <View
                        ...
                        onClick={this.handleConsultButtonClick}
                    >咨询</View>}
                    <Button
                        className={isBuy ? 'btn-pay' : 'btn-pay btn-pay--short'}
                        openType={isRegister || isIOS ? '' : 'getUserInfo'}
