...
import { View, Text, Input, Textarea, Button, Form, PullDownRefresh } from '@tarojs/components';
  ...
  render() {
    ...
    return <View>
                <Form className="editProduct" onSubmit={this.reSure}>
                    ...
                    <Input name="workName" className="" value={this.props.productInfo.workName} />
                    <Text className="title-2">作品简介</Text>
                    <View className="edit-con">
                        <Textarea name="summary" className={this.state.transparent ? 'edit-ctn-transparent' : ''} value={this.props.productInfo.comment} placeholderClass={this.state.transparent ? 'edit-ctn-transparent' : ''} />
                    </View>
