...
class myProduct extends Component {
  ...
  listMenu = i => {
    ...
    this.setState({
      ...
      workName: this.state.myProductList[i].Worksname,
      Comment: this.state.myProductList[i].WorksBrief,
      currentIndex: i
    });
  };
  cancel() {
  ...
  select = (el, i) => {
    ...
    if (i === 0) {
      ...
      // 修改作品信息
      let workName = this.state.workName,
          comment = this.state.Comment;
      this.props.setProductInfo({ workName, comment });
      Taro.navigateTo({
        url: '/pages/editProduct/editProduct?workId=' + this.state.workId
      });
    } else if (i === 3) {
      // 删除作品
      this.setState({
  ...
  render() {
    ...
    return <View className="myProduct">
        ...
        return <View key={i} className="li" onClick={this.listMenu.bind(this, i)}>
                ...
                {this.state.isShowEditToast && <View className="editToast">
            ...
            return <View className="menuList" key={i} onClick={this.select.bind(this, el, i)}>
                ...
                <ModalContainer isShow={isShowModel}>
                    <ModalContainerService onCloseModal={this.toggleModel.bind(this)} top={-160}>
                        {this.state.modalSwitch === 1 && <View>
                                    <View className="modal-tit">温馨提示</View>
                                    <View className="modal-con">需要作品下架后操作</View>
                                    <View className="modal-btn" onClick={this.toggleModel}>我知道了</View>
                                </View>}
                        {this.state.modalSwitch === 2 && <View>
                            <View className="modal-tit">温馨提示</View>
                        ...
                        {this.state.modalSwitch === 3 && <View>
                            <View className="modal-tit">温馨提示</View>
                            <View className="modal-con">是否确认删除作品？</View>
                            <View className="modal-btn2">
                                <Button className="modal-btn2-left" onClick={this.toggleModel}>取消</Button>
                                <Button className="modal-btn2-right" onClick={this.operateWork.bind(this, 2)}>确认</Button>
                            </View>
                        </View>}
