...
import { redirectTo, formatTime, navigateTo } from "../../service/utils.service";
...
class PayChoice extends Component {
  ...
  render() {
    const classList = this.state.productListData.map(item => {
      return <View className={'choice-item choice-item--class ' + (item.GroupId === this.state.activeClassId ? 'choice-item--active' : '')} data-id={item.GroupId} data-time-id={item.PhaseList[0].Phaseid} key={item.GroupId} onClick={this.handleClassSelected}>
                    <Text>{item.GroupName}</Text>
                </View>;
    });

    const timeList = this.filterTimeList().map(item => {
      return <View className={'choice-item choice-item--time ' + (item.Phaseid === this.state.activeTimeId ? 'choice-item--active' : '')} data-id={item.Phaseid} key={item.Phaseid} onClick={this.handleTimeSelected}>
                    <Text space="emsp" decode={true}>{item.Phasename}·{formatTime(item.Starttime)}开课·{item.Summary}</Text>
                </View>;
    });
    ...
    return <View className="wrapper">
                ...
                <View className="choice-content">
                    ...
                    <View>
                        <Input type="number" maxLength="11" className="input-cellphone" placeholderClass="input-cellphone--placeholder" placeholder="手机号仅用于开通课程，无电话销售" value={this.state.cellphone} onInput={this.handleCellPhoneInput} />
                    </View>
                </View>
                <View className="footer">
                    ...
                    <Button className="pay-btn" disabled={!(this.state.isCellphoneValid && this.state.isCheckedProtocol)} onClick={this.handleDoPay}>立即支付</Button>                    
                </View>
