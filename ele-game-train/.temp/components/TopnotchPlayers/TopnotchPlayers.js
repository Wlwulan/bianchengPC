...
class TopnotchPlayers extends Component {
  ...
  render() {
    ...
    return <View className={`TopnotchPlayers`}>
                {goodList.map((el, i) => {
        return <View key={i} className="li" onClick={this.toAuthorize.bind(this, 'allworks', el['StudentId'])}>
                        <View className="img-warp">
                            {el['isPurchase'] === 1 && <Image className="title-frame" mode="widthFix" src="https://appd.knowbox.cn/ss/miniapp/ele_game_register/title-frame.png">
                                </Image>}
