"use strict";
Object.defineProperty(exports, "__esModule", { value: true });class Uploader {
  constructor(t) {
    ...
    this.messageQueue.push(t);const e = this.messageQueue.length;e > 100 && (this.messageQueue = this.messageQueue.slice(e - 100)), this.uploadTimer || (this.uploadTimer = setTimeout(() => {
      this._flush(), this.uploadTimer = null;
    }, 1e3));
  }forceFlush() {
    this.uploadTimer && (clearTimeout(this.uploadTimer), this.uploadTimer = null), this._flush();
  }_flush() {
    this.uploadingQueue = this.messageQueue.slice(), this.messageQueue = [], this.uploadingQueue.length > 0 && wx.request({ url: `${this.url}?stm=${Date.now()}`, header: { "content-type": "application/json" }, method: "POST", data: this.uploadingQueue, success: () => {
        this.messageQueue.length > 0 && this._flush();
      }, fail: () => {
        this.messageQueue = this.uploadingQueue.concat(this.messageQueue);
      } });
  }
}var Utils = { sdkVer: "1.8.7", devVer: 1, guid: function () {
  ...
  }, getOSV: t => `Weixin ${t}`, isEmpty: t => {
    for (var e in t) if (t.hasOwnProperty(e)) return false;return true;
  } };class Page$1 {
  constructor() {
    this.queries = {};
  }touch(t) {
...
}const eventTypeMap = { tap: ["tap", "click"], longtap: ["longtap"], input: ["input"], blur: ["change", "blur"], submit: ["submit"], focus: ["focus"] },
  ...
  constructor(t) {
    ...
    var e = t.type,
      ...
      var a = getHandle(o._vnode, r, eventTypeMap[e] || [e]);if (a.length) {
  ...
  constructor(t) {
    ...
    this.growingio.vue && (t = t.$mp.page), this.growingio.taro && (t = t.$scope);var i = {};this._observer && this._observer.disconnect(), this._observer = t.isComponent ? t.createIntersectionObserver({ observeAll: true }) : wx.createIntersectionObserver(t, { observeAll: true }), this._relative = e ? this._observer.relativeTo(e) : this._observer.relativeToViewport(), this._relative.observe(".growing_collect_imp", t => {
      t.id && !i[t.id] && (this.track(t.dataset.gioTrack && t.dataset.gioTrack.name, t.dataset.gioTrack && t.dataset.gioTrack.properties), i[t.id] = true);
    });
  }appListener(t, e, i) {
    ...
    if ("handleProxy" === e && this.growingio.vueRootVMs && this.growingio.vueRootVMs[this.currentPage.path]) {
      let i = new VueProxy(this.growingio.vueRootVMs[this.currentPage.path]).getHandle(t);i && (e = i);
    ...
    if (null !== t && undefined !== t && 0 !== t.length) {
      var i = { t: "cstm", ptm: this.currentPage.time, p: this.currentPage.path, q: this.currentPage.query, n: t };null !== e && "object" == typeof e && (i.var = e), this._sendEvent(i);
    }
  }identify(t, e) {
    ...
    if (t && t.length > 0) {
      let e = t[0].split("\n");if (e && e.length > 1) {
        let t = e[1].split(";");if (t && t.length > 1) {
          let i = t[1].match(/at ([^ ]+) page (.*) function/),
              s = { key: e[0], error: t[0] };i && i.length > 2 && (s.page = i[1], s.function = i[2]), this._sendEvent({ t: "cstm", ptm: this.currentPage.time, p: this.currentPage.path, q: this.currentPage.query, n: "onError", var: s });
        }
      }
    }
  }sendPage(t) {
  ...
  constructor(t) {
    ...
    return new Promise(t => {
      wx.getNetworkType({ success: e => t(e), fail: () => t(null) });
    });
  }getPageTitle(t) {
    var e = "";try {
      if (t.data.title && t.data.title.length > 0 && (e = Array.isArray(t.data.title) ? t.data.title.join(" ") : t.data.title), 0 === e.length && __wxConfig) {
        if (__wxConfig.tabBar) {
          var i = __wxConfig.tabBar.list.find(e => e.pathPath == t.route || e.pagePath == `${t.route}.html`);i && i.text && (e = i.text);
        }if (0 == e.length) {
    ...
    return new Promise(t => {
      wx.getLocation({ success: t, fail: function () {
          return t(null);
        } });
    });
  }
}var VdsInstrumentAgent = { defaultPageCallbacks: {}, defaultAppCallbacks: {}, appHandlers: ["onShow", "onHide", "onError"], pageHandlers: ["onLoad", "onShow", "onShareAppMessage", "onTabItemTap", "onHide"], actionEventTypes: ["tap", "longpress", "blur", "change", "submit", "confirm", "getuserinfo", "getphonenumber", "contact"], originalPage: Page, originalApp: App, originalComponent: Component, originalBehavior: Behavior, hook: function (t, e) {
    return function () {
      var i,
    ...
    for (var e in t) "function" == typeof t[e] && (t[e] = this.hook(e, t[e]));return t._growing_app_ && VdsInstrumentAgent.appHandlers.map(function (e) {
    ...
    if (t.methods) {
      let e = t.methods;for (let i in e) if ("function" == typeof e[i] && -1 != VdsInstrumentAgent.pageHandlers.indexOf(i)) {
        const s = e[i];t.methods[i] = function () {
          return VdsInstrumentAgent.observer.pageListener(this, i, arguments), s.apply(this, arguments);
        };
      }
    }return t;
  }, instrumentComponent: function (t) {
    if (t.methods) {
      let e = t.methods;for (let i in e) "function" == typeof e[i] && (t.methods[i] = this.hookComponent(i, e[i]));
    }return t;
  ...
  const e = {};for (let i of Reflect.ownKeys(t)) e[i] = Object.getOwnPropertyDescriptor(t, i);return e;
});class GrowingIO {
  constructor() {
    ...
    if (this.forceLogin) for (var e of (this.weixin.uid = t, this.forceLogin = false, this.uploadingMessages)) e.u = t, this._upload(e);
  }upload(t) {
    ...
    let e = this;const i = t.createComponent;t.createComponent = function (t, s) {
      let n = t;for (; n && n.prototype;) {
        const i = Object.keys(Object.getOwnPropertyDescriptors(n.prototype) || {});for (let s = 0; i.length > s; s++) if (i[s].startsWith("func__")) {
          const r = n.name,
                o = i[s].slice(6);e.taroRootVMs[i[s]] = r + "_" + ("" + t.prototype[i[s]]).match(/this\.__triggerPropsFn\(\"(.+)\",/)[1] + "_" + o;
        }n = Object.getPrototypeOf(n);
      }const r = i(t, s);return s && VdsInstrumentAgent.instrumentTaroPageComponent(r), r;
    };
  }_proxyVue(t) {
    if (undefined !== t.mixin) {
      let e = this;t.mixin({ created: function () {
          if (!this.$options.methods) return;const t = Object.keys(this.$options.methods);for (let e of Object.keys(this)) 0 > t.indexOf(e) || "function" != typeof this[e] || (Object.defineProperty(this[e], "proxiedName", { value: e }), Object.defineProperty(this[e], "isProxied", { value: true }));
        }, beforeMount: function () {
          let t = this.$root;t.$mp && "page" === t.$mp.mpType && t.$mp.page && (e.vueRootVMs[t.$mp.page.route] = t);
        } });
    }
  }
...
};console.log("init growingio...");const GioPage = VdsInstrumentAgent.GrowingPage,
